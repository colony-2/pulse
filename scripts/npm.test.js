...
const { assetName, download, install } = require("../npm/cortex/scripts/postinstall");
...
test("platform names match c2j's four release targets", () => {
  ...
  ]) assert.equal(assetName("1.2.3", platform, arch), `cortex_1.2.3_${suffix}.tar.gz`);
...
function fixture(t, script = "#!/bin/sh\nprintf '%s\\n' \"$@\"\nexit 7\n") {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-npm-test-"));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ version: "1.2.3" }));
  fs.mkdirSync(path.join(root, "bin"));
  fs.copyFileSync("npm/cortex/bin/cli.js", path.join(root, "bin/cli.js"));
  fs.writeFileSync(path.join(root, "cortex"), script, { mode: 0o755 });
  const archive = path.join(root, assetName("1.2.3"));
  execFileSync("tar", ["-czf", archive, "-C", root, "cortex"]);
  let sum = crypto.createHash("sha256").update(fs.readFileSync(archive)).digest("hex");
  return {
    root,
    corrupt: () => { sum = "0".repeat(64); },
    fetch: async (url, output) => {
      assert.ok(url.startsWith("https://github.com/colony-2/cortex/releases/download/v1.2.3/"));
...
test("verified install forwards arguments and exit code; corruption preserves existing binary", async (t) => {
  ...
  const binary = path.join(f.root, "vendor/cortex");
