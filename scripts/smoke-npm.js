...
async function main() {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-pack-smoke-"));
  try {
    const archive = path.resolve(releaseDir, `colony2-cortex-${version}.tgz`);
    execFileSync("npm", ["install", "--prefix", dir, "--ignore-scripts", "--no-audit", "--no-fund", archive], { stdio: "inherit" });
    const root = path.join(dir, "node_modules/@colony2/cortex");
    ...
    if (output !== `cortex ${version}`) throw new Error(`Unexpected npm-installed version: ${output}`);
